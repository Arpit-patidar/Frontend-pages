...
const run = () => {
    ...
    e.x += (ax + pointer.x - e.x) / 2;
    e.y += (ay + pointer.y - e.y) / 2;
    
    for (let i = 1; i < N; i++) {
        let e = elems[i];
        let ep = elems[i - 1];
        const a = Math.atan2(e.y - ep.y, e.x - ep.x);
        e.x += (ep.x - e.x + (Math.cos(a) * (100 - i)) / 5) / 2;
        e.y += (ep.y - e.y + (Math.sin(a) * (100 - i)) / 5) / 2;
    ...
    frm += 0.005;
    if (rad > 60) {
        pointer.x += (width / 2 - pointer.x) * .5;
        pointer.y += (height / 2 - pointer.y) * .5;
    }
}; 
